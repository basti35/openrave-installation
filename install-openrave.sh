...
if [ ${UBUNTU_VER} = '14.04' ] || [ ${UBUNTU_VER} = '16.04' ]; then
      ...
      -DRAPIDJSON_ROOT_DIR=${REPO_FOLDER}/deps \
      -DOSG_DIR=${REPO_FOLDER}/deps \
      -DCMAKE_INSTALL_PREFIX=${REPO_FOLDER}/deps \
      -Wno-dev ..
  	cmake -DODE_USE_MULTITHREAD=ON -DUSE_PYBIND11_PYTHON_BINDINGS:BOOL=TRUE	-DBoost_NO_BOOST_CMAKE=1 -DRAPIDJSON_ROOT_DIR=${REPO_FOLDER}/deps -DOSG_DIR=${REPO_FOLDER}/deps -DCMAKE_INSTALL_PREFIX=${REPO_FOLDER}/deps -Wno-dev ..
fi
exit
make -j `nproc`
#sudo make install
